...
class TransitiveClosure:
    ...
    def add(self, a, b):
        ...
        prev, next   = None, None
        nodea = self.__nodes.get(keya)
        nodeb = self.__nodes.get(keyb)
        if not nodea:
            nodea = Node(keya, itema)
            self.__nodes[keya] = nodea
        if not nodeb:
            nodeb = Node(keyb, itemb)
            self.__nodes[keyb] = nodeb
        # next
